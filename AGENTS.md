...
- `src/arcter/cli.py`: Typer CLI entrypoint (717 lines), exposed via the `arcter` console script. Defines the root `app`, `config_app` subgroup, and `account_app` subgroup. Contains helper functions for table rendering, date validation, invoice cycle calculation, and transaction formatting.
...
- `src/arcter/pluggy.py`: Pluggy API integration (617 lines) — authentication, item updates, balance retrieval, credit card details, account listing, and transaction history via `httpx`. Includes credential/item resolution with environment, keyring, and config fallbacks. All HTTP calls go through a central `_request_json` helper with unified timeout/error handling and pagination support.
...
- `tests/test_account_cli.py`: integration tests for account CLI commands (50 tests). Uses `monkeypatch` to stub `arcter.cli.pluggy.*` and `arcter.cli.credentials.*` functions. Covers login/logout, update, balance, goal, credit, and transactions commands.
...
- `tests/test_pluggy.py`: unit tests for the Pluggy module (22 tests). Uses `monkeypatch` to stub `_request_json`. Covers balance filtering/parsing, credit card parsing, account listing, transaction pagination/parsing, env-based orchestrators, and credential/item-id fallback resolution.
- `prompts/`: feature specification documents used during development:
  ...
  - `04-spending-summary.md`: spec for `arcter account spending` (**not yet implemented**).
...
- `uv run pytest`: run the test suite (92 tests across 4 files).
...
- **`test_account_cli.py`** (50 tests): account login/logout, update, balance, goal, credit, and transactions commands — happy paths, env-var/keyring/config fallback behavior, credential/item-id validation errors, HTTP error propagation, timeout handling, date filtering, transaction type/account type filtering, category exclusion (config + CLI), limit option, and edge cases (no accounts, nil balances, zero salary).
- **`test_credentials.py`** (8 tests): keyring store/load/delete behavior, empty/missing value handling, and `CredentialError` wrapping.
- **`test_pluggy.py`** (22 tests): balance filtering/parsing, invalid payload handling, env/keyring credential resolution, arg/env/config item-id resolution, credit card data parsing (including missing/non-dict creditData), account listing, transaction pagination, transaction field extraction, unparseable amounts, missing fields, and account-type filtering in orchestrators.
