...
- `src/arcter/commands/account.py`: account command group (`login`, `logout`, `update`, `balance`, `credit`, `goal`, `salary`, `transactions`, `spending`) plus transaction output format enum and command-local error exit helper. Includes salary sync helpers, interactive transaction picker, automatic last-month filter fallback for `account salary --sync`, and silent salary auto-sync hook in `transactions`.
...
| `arcter account salary [ITEM_ID]` | `item_id`: optional, `--sync`: sync from transactions using configured filters (current month with automatic last-month fallback), `--set`: manual salary value | Manage salary via manual set or transaction-based sync with interactive picker fallback |
...
- **`test_account_cli.py`** (80 tests): account login/logout, update, balance, goal, credit, salary, transactions, and spending commands — happy paths, env-var/keyring/config fallback behavior, credential/item-id validation errors, HTTP error propagation, timeout handling, date filtering/defaults, transaction type/account type filtering, category exclusion (config + CLI), transactions output format coverage (`table`, `csv`, `json`), salary sync interactive flow (`--sync`) with automatic last-month fallback and silent transactions sync, top-N limits, and edge cases (no accounts, nil balances, zero salary).
